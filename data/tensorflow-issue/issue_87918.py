...
numeric_columns_for_model = [col for col in numeric_columns if col != "Bet_Result"]
for train_chunk, _ in stream_split_data(parquet_files, categorical_columns, numeric_columns):
    X_train = train_chunk.drop(columns=["Bet_Result"]) # Do not fit target variable
...
# Now, fit it on a dummy DataFrame with the correct column names.
...
def preprocess_dense_batches(parquet_files, categorical_columns, numeric_columns, batch_size, end_idx=None):
    ...
    label_mapping = {0.5: 0, 0: 1, 1: 2}  # Map pushes, misses, and hits to integers

    current_idx = 0
    for train_chunk, _ in stream_split_data(parquet_files, categorical_columns, numeric_columns):
        
        y_chunk = train_chunk.pop("Bet_Result").apply(lambda x: label_mapping[x]).values.astype(int)  # Extracts y (target variable) and drops column from the rest of the data in one step
...
output = Dense(3, activation='softmax')(x)  # 3 neurons, softmax for multi-class classification (hit, miss, push)

print("Compiling the model...")

# Compile the model
model = Model(inputs=input_all_nfl_data, outputs=output)
