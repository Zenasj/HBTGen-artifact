...
def incidence_matrix_(adj_sparse, edge_index, num_nodes):
    ...
    return torch.sparse_coo_tensor(
        indices, values, (edge_number_unique, num_nodes)
    ).transpose(0, 1)
...
def test_sparse_mm():
    ...
    adj_matrix = torch.Tensor([[0, 0, 0, 0, ], [0, 0, 1, 1, ], [0, 1, 0, 0, ], [0, 1, 0, 0, ]]).to_sparse_coo()
    num_nodes = 4
...
result= tensor([[-3.,  0.], [ 0., -3.]])
expected_result=tensor([[-2.,  0.], [ 0., -2.]])