...
class Model0():
    def forward(self, *args):
        min_1 = args[1].min(0)
        getattr_1 = min_1.values
        min_2 = torch.min(getattr_1, args[0])
        mean = min_2.mean(0)
        gelu = torch._C._nn.gelu(mean)
...
class Model1():
    def forward(self, *args):
        ...
        mean = min_2.mean(0)
        gelu = torch._C._nn.gelu(mean)
        floor = torch.floor(gelu)
        atan = torch.atan(floor)
        sub = torch.sub(gelu, gelu)
        return (atan, sub)
...
model_1 = Model1()
output_names_1 = ['v3_0', 'v4_0']

data_0 = np.array([[5.859193]], dtype=np.float32)
data_1 = np.array(4.8227854, dtype=np.float32)
