...
def warmed_up(iters = 1):
    random.seed(0)
    np.random.seed(0)
    torch.manual_seed(0)
    torch.cuda.manual_seed(0)
    conv2d = nn.Conv2d(3, 8, 3, bias=False).to(device)
    dconv2d = DConv2d(3, 8, 3, bias=False).to(device)
    optimizer = optim.SGD(conv2d.parameters(), lr = 0.01, momentum=0.9)
    x = torch.randn((64,3,32,32), device=device, requires_grad=True)
