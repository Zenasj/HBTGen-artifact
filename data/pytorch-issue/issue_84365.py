...
from flair.embeddings.transformer import TransformerOnnxEmbeddings
import torch
...
example_tensors = model.embeddings.prepare_tensors(sentences)
...
torch_wrapper = TorchWrapper(model.embeddings)

input_tensors = [example_tensors["input_ids"],
        example_tensors["lengths"],
        example_tensors["attention_mask"],
        example_tensors["overflow_to_sample_mapping"],
        example_tensors["word_ids"]]

ONNX_OPSET_VERSION = 17

def get_outputs_sizes_torch(torch_model, input_tensors):
...
output_sizes = get_outputs_sizes_torch(torch_wrapper, input_tensors)
