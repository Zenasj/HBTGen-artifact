...
class Model0():
    def forward(self, *args):
        mul = torch.mul(args[1], args[0])
        squeeze = mul.squeeze(1)
        tan = torch.tan(squeeze)
        ceil = torch.ceil(tan)
        softmax = torch.softmax(ceil, dim = 0)
        return (softmax)

class Model1():
    def forward(self, *args):
        mul = torch.mul(args[1], args[0])
        cat = torch.cat((args[0], args[0], mul, mul), dim = 2)
        squeeze = mul.squeeze(1)
        tan = torch.tan(squeeze)
        ceil = torch.ceil(tan)
        softmax = torch.softmax(ceil, dim = 0)
...
model_1 = Model1()
output_names_1 = ['v4_0', 'v5_0']

data_0 = np.random.normal(5, 1, size=(34, 1, 17)).astype(np.float16)
data_1 = np.random.normal(5, 1, size=(1,)).astype(np.float16)
