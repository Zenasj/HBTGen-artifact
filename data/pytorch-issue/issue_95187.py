...
args = [((1, 2), (2, 1), torch.int32, 'cpu', False), ((0, 1), (1, 1), torch.int64, 'cpu', False)]
...
class Repro(torch.nn.Module):
    ...
    def forward(self, op_inputs_dict_input_ : torch.Tensor, op_inputs_dict_index_ : torch.Tensor):
        gather = torch.gather(input = op_inputs_dict_input_, dim = 1, sparse_grad = True, index = op_inputs_dict_index_);  op_inputs_dict_input_ = op_inputs_dict_index_ = None
        return (gather,)
...
class Repro(torch.nn.Module):
    ...
    def forward(self, op_inputs_dict_input_ : torch.Tensor, op_inputs_dict_index_ : torch.Tensor):
        gather = torch.gather(input = op_inputs_dict_input_, dim = 1, sparse_grad = True, index = op_inputs_dict_index_)
        return (gather,)
