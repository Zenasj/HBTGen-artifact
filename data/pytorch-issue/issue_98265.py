...
def create_sequence_gen_model(embed_dim=1536, beam_size=8, generate_size=1024):
    underlying_model = create_model(embed_dim)
    return SequenceGenerator(underlying_model, beam_size, generate_size).eval().cuda()

def benchmark_sequence_with_compile_new():
    embed_dim = 1536
    beam_size = 4
    generate_size = 1
    ...
    model_inc = (
        SequenceGenerator(create_model(embed_dim), beam_size, generate_size)
        .eval()
        .cuda()
    )
    script_optimized_model_inc = (
        SequenceGenerator(
            torch.jit.optimize_for_inference(torch.jit.script(create_model(embed_dim))),
            beam_size,
            generate_size,
        )
        .eval()
    ...
    input_tensor = torch.randint(1, 128, (1, 2)).cuda()
    for _ in tqdm(range(4)):
        print("model_inc")
        model_inc(input_tensor)
        print("script_optimized_model_inc")
        script_optimized_model_inc(input_tensor)
        print("compiled_inc")
        compiled_inc(input_tensor)
    results = []
    benchmarks = [
        benchmark.Timer(
            stmt="model_inc(input_tensor)",
            globals={"model_inc": model_inc, "input_tensor": input_tensor},
            num_threads=num_threads,
            label="Original Model Incremental",
            description="Original Model Incremental",
        ),
        benchmark.Timer(
            stmt="compiled_fixed(input_tensor)",
            globals={
                "compiled_fixed": compiled_inc,
                "input_tensor": input_tensor,
            },
            num_threads=num_threads,
            label="Compiled Fixed Model",
            description="Compiled Fixed Model",
        ),
    ]
    print("bench")
    for bm in benchmarks:
        results.append(bm.blocked_autorange(min_run_time=5))
        print(results[-1])
    return results

compare = benchmark.Compare(benchmark_sequence_with_compile_new())
compare.print()