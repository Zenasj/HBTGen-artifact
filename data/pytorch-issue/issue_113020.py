...
p0 = torch.tensor(6, dtype=torch.uint8)
...
class Model0(torch.nn.Module):
    ...
    def forward(self, *args):
        ...
        getitem = _args[0]
        getitem_1 = _args[1]
        sub = torch.sub(getitem, v5_0)
        mul = torch.mul(getitem_1, sub)
        to = mul.to(dtype = torch.int32)
        return (to)
...
class Model1(torch.nn.Module):
    ...
    def forward(self, *args):
        ...
        getitem = _args[0]
        getitem_1 = _args[1]
        sub = torch.sub(getitem, v3_0)
        mul = torch.mul(sub, getitem_1)
        to = mul.to(dtype = torch.int32)
        return (to)
