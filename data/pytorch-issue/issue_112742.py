...
def forward(x, device):
  x = torch.mm(mat2=x, input=torch.rand([8,8],dtype=torch.float32).to('cpu'))        
  return x
input_tensor = torch.rand([8,8],dtype=torch.float32).to('cpu')
...
def forward(x, y, device):
  x = torch.mm(mat2=x.to(device), input=y.to(device))        
  return x
a =torch.rand([8,8],dtype=torch.float32)
b = torch.rand([8,8],dtype=torch.float32)

no_op_info = forward(a, b, 'cpu')
print("build succeded")

op_info = torch.compile(forward, mode='max-autotune',fullgraph=False,dynamic=True)(a, b, 'cuda')
