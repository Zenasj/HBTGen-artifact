...
print('Are these both log(det(A))?', bool(logdet1 - logdet2 < 1e-8))  # True

logdet1.backward()
logdet2.backward()

inv_mat = mat.inverse()  # the analytical solution
print('Does Way 1 yield A^{-1}?', bool(torch.norm(mat.grad - inv_mat) < 1e-8))  # True
print('Does Way 2 yield A^{-1}?', bool(torch.norm(mat_clone.grad - inv_mat) < 1e-8))  # False