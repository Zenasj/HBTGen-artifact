import requests
import pickle

def load_pickle_from_url(url) :
  # Download the file
  response = requests.get(url)
  if response.status_code == 200:
      # Load the content of the file
      data = pickle.loads(response.content)

      return data
  else:
      print("Failed to download the file")
      return None

import torch
import torch.nn as nn
import traceback
import os, pickle
import numpy as np

pickle_url = "https://github.com/GwiHwan-Go/repo/raw/main/issues/pickles/issue_117544.pkl"
inputs = load_pickle_from_url(pickle_url)

if inputs is None:
    raise ValueError("Pickle file could not be downloaded. Please check the URL.")

class Model(nn.Module):
    def __init__(self):
        super().__init__()
    def forward(self, out):

        
        out = torch.sinh(input=out)        
        out = torch.relu(input=out)        
        return out

x = inputs[0]['input']
model = Model().to(torch.device('cpu'))
eag = model(x)
opt = torch.compile(model.forward, mode='max-autotune')(x)

np.testing.assert_allclose(eag.to('cpu').numpy(), opt.to('cpu').numpy(), rtol=1e-3, atol=1e-3, equal_nan=True, verbose=True)

import torch

def test(with_cuda):
    def fn(x):
        return torch.relu(x)

    device = "cuda" if with_cuda else "cpu"
    x = torch.tensor(-torch.inf, device=device)

    eager = fn(x)
    opt_dynamo = torch.compile(fn, backend='eager')(x)
    opt_inductor = torch.compile(fn, backend='inductor')(x)

    print(f"device={x.device}")
    print(f"  eager={eager}")
    print(f"  dynamo={opt_dynamo}")
    print(f"  inductor={opt_inductor}")

test(with_cuda=True)
test(with_cuda=False)