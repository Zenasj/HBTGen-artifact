...
def register_custom_nms_op(opset_version=10):
    ...
    def symbolic_nms(g, boxes, scores, iou_threshold, max_output_boxes):
        ...
        #                       center_point_box_i=1)  # faster rcnn does not use center_point
        idx = select(g, nms_out, 1, g.op('Constant', value_t=torch.tensor([2], dtype=torch.long)))
        return view(g, idx, (-1,))

    from torch.onnx import register_custom_op_symbolic
    register_custom_op_symbolic('roi_ops::nms', symbolic_nms, opset_version)