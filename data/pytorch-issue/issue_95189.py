...
args = [((4, 16, 32, 16), (8192, 512, 16, 1), torch.bfloat16, 'cpu', True), ((1,), (1,), torch.bfloat16, 'cpu', False), ((150,), (1,), torch.int64, 'cpu', False), ((150,), (1,), torch.int64, 'cpu', False), ((150,), (1,), torch.int64, 'cpu', False), ((150,), (1,), torch.int64, 'cpu', False)]
args = [rand_strided(sh, st, dt, dev).requires_grad_(rg) for (sh, st, dt, dev, rg) in args]

def forward(self, op_inputs_dict_input_ : torch.Tensor, op_inputs_dict_values_ : torch.Tensor, op_inputs_dict_indices_0_ : torch.Tensor, op_inputs_dict_indices_1_ : torch.Tensor, op_inputs_dict_indices_2_ : torch.Tensor, op_inputs_dict_indices_3_ : torch.Tensor):
        index_put = torch.index_put(accumulate = True, input = op_inputs_dict_input_, values = op_inputs_dict_values_, indices = [op_inputs_dict_indices_0_, op_inputs_dict_indices_1_, op_inputs_dict_indices_2_, op_inputs_dict_indices_3_]);  op_inputs_dict_input_ = op_inputs_dict_values_ = op_inputs_dict_indices_0_ = op_inputs_dict_indices_1_ = op_inputs_dict_indices_2_ = op_inputs_dict_indices_3_ = None
        return (index_put,)
...
class BasicModule(torch.nn.Module):
    ...
    def forward(self, input, dim, index):
        res = torch.index_select(input=input, dim=dim, index=index)
        return res

if __name__ == "__main__":
    input= torch.tensor([[[ 0., -3.,  1.],
         [ 0.,  5.,  1.],
         [-3., -1.,  3.],
         [ 1.,  5.,  0.]]], dtype=torch.bfloat16, requires_grad=True)
    dim = -2
    index =torch.tensor([3, 0], dtype=torch.int64)
    model = BasicModule()
    model = torch.compile(model)
    fwd_res = model(input, dim, index)
    bwd_tensor = torch.randn(fwd_res.shape, dtype=torch.bfloat16)
    fwd_res.backward(bwd_tensor)
    print(fwd_res, input.grad)