# torch.randint(50304, (8, 1024), dtype=torch.int64)
import torch
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x_BTE):
        qkv = self.attn_proj(x_BTE).split(x_BTE.size(-1), dim=-1)
        split_attn_head = lambda z: z.unflatten(-1, [-1, self.d_head]).transpose(1, 2)
        q_BHTD, k_BHTD, v_BHTD = map(split_attn_head, qkv)

        # Use math SDPA to avoid Inductor shape assertion failure
        with torch.backends.cuda.sdp_kernel(enable_math=True, enable_flash=False, enable_mem_efficient=False):
            o_BHTD = F.scaled_dot_product_attention(
                q_BHTD, k_BHTD, v_BHTD, dropout_p=0.0, is_causal=True
            )

        o_BTE = o_BHTD.transpose(1, 2).flatten(start_dim=-2)
...
def my_model_function():
    return MyModel(
        ...
        n_layers=1,
        d_embd=768,
        n_heads=12  # Passed via **kwargs to CausalSelfAttention
    )

def GetInput():
    return torch.randint(50304, (8, 1024), dtype=torch.int64, device='cuda:0')
