...
args = [((64,), (1,), torch.float32, 'cpu', False)]
...
class Repro(torch.nn.Module):
    ...
    def forward(self, in_out_tensor : torch.Tensor):
        squeeze_ = in_out_tensor.squeeze_(dim = 1);  in_out_tensor = None
        return (squeeze_,)
...
class BasicModule(torch.nn.Module):
    ...
    def forward(self, tensor):
        res = tensor.squeeze_(dim=1)
        return res


if __name__ == "__main__":
    tensor = torch.randn([10, 1], dtype=torch.float32)
    model = BasicModule()
