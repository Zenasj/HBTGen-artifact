...
def dist(y: list[torch.Tensor], scanned_x: list[torch.Tensor]):
    y = y[0] # change 0: y and x would be pytree
    scanned_x = scanned_x[0]
    sub = y - scanned_x.reshape((1, -1))
    sq = sub * sub
    rd = torch.sqrt(sq.sum(axis=1))
    # clone --> UnsupportedAliasMutationException:
    # Combine_fn might be aliasing the input!
    return [y.clone()], [rd] # change 1: pytree matching 


class ModuleWithControlFlowLoopScan(torch.nn.Module):

    def forward(self, x, y):
        carry, out = torch._higher_order.scan(dist, [y], [x]) # change 2: remove additional_inputs 
        return out